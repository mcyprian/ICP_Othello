...
Game& GameManager::getGame() { return *this->game; }

void GameManager::initNewGame(string game_name, GameMode mode, int size,
                              string name1, Color color, Difficulty d) {
  if (this->initialized)
    runtime_error(string(__func__) +
                  string("Game can be initialized only once.\n"));
...
void GameManager::initNewGame(string game_name, GameMode mode, int size,
                              string name1, Color color, string name2) {
  if (this->initialized)
    runtime_error(string(__func__) +
                  string("Game can be initialized only once.\n"));
  this->game = new Game(game_name, mode, size);
...
void GameManager::saveGame() {
  if (!this->initialized)
    runtime_error(string(__func__) + string("Game was not loaded\n"));
...
void GameManager::removeGame(string name) {
  if (!this->initialized)
    runtime_error(string(__func__) + string("Game was not loaded\n"));
  if (saved[name] != this->game) delete saved[name];
  saved.erase(name);
}

void GameManager::loadGame(string name) {
  if (this->game != nullptr) this->saved[this->game->getName()] = this->game;
  if (this->saved.count(name) == 0)
    runtime_error(string(__func__) +
                  string("Game " + name + " doesn't exist.\n"));
  else
