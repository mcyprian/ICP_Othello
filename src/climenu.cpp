...
CLIMenu::CLIMenu(string title) { this->title = title; }

void CLIMenu::addEntry(string entry) { this->entries.push_back(entry); }
...
unsigned CLIMenu::prompt(string title) {
  ...
  if (title.empty()) title = this->title;
  ...
  while (!satisfied) {
    ...
    try {
      ...
      if (num_choice < 0 ||
          static_cast<unsigned>(num_choice) >= this->entries.size())
