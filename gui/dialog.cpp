 ...
 * @section DESCRIPTION
 *
 * Implementation of graphical variant of user interface for the specific game
...
Dialog::Dialog(GameManager *gm, QWidget *parent)
  ...
  this->ui->backward_button->setGeometry(
      275, this->ui->graphicsView->y() + background_size + 20, 120, 81);
  this->ui->forward_button->setGeometry(
      400, this->ui->graphicsView->y() + background_size + 20, 120, 81);
...
void Dialog::drawScene() {
  this->ui->game_name->setText(
      QString::fromStdString(this->gm->getGame().getName()));
  QString background;
  ...
  for (int i = 0; i < this->grid_size; i++) {
    for (int j = 0; j < this->grid_size; j++) {
      ...
      this->connect(this->ggrid[i][j], SIGNAL(cellSelected(int, int)), this,
                    SLOT(cellSelected(int, int)));
      this->connect(this->ggrid[i][j], SIGNAL(cellMoved(int, int)), this,
                    SLOT(cellMoved(int, int)));
      this->connect(this->ggrid[i][j], SIGNAL(cellLeft(int, int)), this,
                    SLOT(cellLeft(int, int)));
    }
  }

  // load player1 avatar image and name
  this->ui->player1_image->setPixmap(
      QPixmap(":/image/images/black_avatar.png"));
  this->ui->player1_name->setText(
      QString::fromStdString(this->gm->getGame().getPlayer1()->name));
...
void Dialog::setVersus() {
  this->ui->player2_image->setPixmap(
      QPixmap(":/image/images/white_avatar.png"));
  this->ui->player2_name->setText(
      QString::fromStdString(this->gm->getGame().getPlayer2()->name));
}
...
void Dialog::checkEnd() {
  if (this->gm->getGame().existMove() == FAILURE) {
    this->updateScore();
    QString winner =
        QString::fromStdString(this->black_count > this->white_count
                                   ? this->gm->getGame().getPlayer1()->name
                                   : this->gm->getGame().getPlayer2()->name);
    QMessageBox::information(this, tr("END OF THE GAME"),
                             "Player: " + winner + " have won!");
  }
}
...
void Dialog::cellLeft(int x, int y) {
  GUIDisk *current = this->ggrid[x][y]->getDisk();
  if (current->isTransparent()) current->setVisible(false);
}
